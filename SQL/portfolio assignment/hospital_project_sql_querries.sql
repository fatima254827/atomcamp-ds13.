...
CREATE SCHEMA hospital_mnagement_system;
USE hospital_mnagement_system;
...
-- lets insert data into these tables 

-- data insertion 
INSERT INTO hospitals(hospital_id, name, address, phone) 
VALUES (1, 'Green Valley Medical Center', '123 Elm St, Springfield', '555-123-4567'),
...
VALUES (1, 'Emily Carter', 'Cardiology', 1, 'ecarter@greenvalley.com'),
...
VALUES (1, 41, 26, '2025-04-16', 'Follow-up'),
...
VALUES (1, 'Aspirin', 'Pain reliever and anti-inflammatory'),
...
select r.room_no,  d.name, r.capacity as capacity_of_room
from rooms r
...
select p.name, p.phone, a.appointment_date as appointment_in_august
from  patients p
...
select d.doctor_id, d.name, p.prescription_id
from doctors d
...
select p.name, p.patient_id, COUNT(DISTINCT pr.medication_id) AS meds_count
from patients p 
...
-- ------------------------------------------advance querries-------------------------------------------------
-- 1)Rank doctors by number of appointments (within their hospitals). 
SELECT doctor_id,doctor_name, hospital_name,  total_appointments,
       RANK() OVER (PARTITION BY hospital_id ORDER BY total_appointments DESC) AS rank_in_hospital
FROM (
select d.name as doctor_name, count(a.appointment_id) as total_appointments,  d.hospital_id, d.doctor_id , h.name as hospital_name
from doctors d 
left join  appointments a on a.doctor_id = d.doctor_id
JOIN hospitals h ON h.hospital_id = d.hospital_id
group by d.doctor_id, d.name, d.hospital_id, h.name
)t;

-- 2) List the last 2 appointments for every patient. 
select *
from ( select * , row_number() over(partition by patient_id order by appointment_date desc) as rn
from appointments ) x
where rn <= 2
order by patient_id, appointment_date DESC;

-- 3) For all the emergency appointments, show patient name, date of birth as well as age-group according to the following: 
-- ○ 18 or below - Pediatric 
-- 19 to 64 - Adult 
-- 65 or above - Geriatric 
select p.name, p.dob, a.reason, 
case 
...
-- 4) Out of all the consultation appointments with cardiologists, break down how many are by pediatric, adult and geriatric patients each (age criteria is the same as above).  
Select age_group, COUNT(*) AS count_consults
FROM (
    select p.patient_id,
           CASE
         When TIMESTAMPDIFF(YEAR, p.dob, CURDATE()) <= 18 THEN 'Pediatric'
    ...
    JOIN patients p ON a.patient_id = p.patient_id
    WHERE LOWER(a.reason) = 'consultation' AND LOWER(d.specialty) = 'cardiology'
) sub
GROUP BY age_group;

-- 5) Every medication has a prescription frequency, i.e. the number of times it has been prescribed. Find all medications with the top three prescription frequencies
-- (for example  if the top 3 frequencies are 8,7 and 6, then list all medications prescribed these number of times).

select m.medication_id, m.name, COUNT(p.prescription_id) AS frequency
from prescriptions p
join medications m ON p.medication_id = m.medication_id
group by  m.medication_id, m.name
having COUNT(p.prescription_id) IN (  select DISTINCT frequency
    from (
       select COUNT(prescription_id) AS frequency
       from prescriptions
...
-- 6) Show all hospitals with the lowest doctor count.
...
from doc_counts
where  doctor_count = (select MIN(doctor_count) from doc_counts);

-- 7) Out of the cardiology departments in each hospital, find out which hospital(s)'s cardiology department has the most number of rooms. 
with cardio_rooms as (
...
select *
from cardio_rooms
where room_count = (select max(room_count) from cardio_rooms);

-- 8) find out the difference in days between the appointment dates of returning patients 

select *
from (select patient_id,
           appointment_date as current_appointment,
...
order by patient_id, current_appointment;

-- ------------------------------------------------bonus queries ---------------------------------------

-- 1) Top hospitals with the most specialists in a particular field (e.g., cardiology) 

select h.name, count(d.doctor_id) as cardiologists
from hospitals h
join doctors d on h.hospital_id = d.hospital_id
where lower(d.specialty) = 'cardiology'
group by h.hospital_id, h.name
order by cardiologists desc;
 
 -- 2) Average room capacity per department or per hospital. 
 
 SELECT d.name AS department, AVG(r.capacity) AS avg_capacity
FROM departments d
JOIN rooms r ON d.department_id = r.department_id
GROUP BY d.department_id, d.name;
 
 -- 3) the trend of appointments over time (daily/weekly/monthly). 
...
-- 4) Most frequent appointment reasons. 
